...
com R sendo uma matriz diagonal onde $R_{kk} = y_k(1 - y_k)$.
...
\begin{center}
	...
	\begin{split}\label{eq:softmax_derivative}
		\frac{\partial y_k}{\partial a_j} = \frac{\partial \frac{e^{a_k}}{\sum_i e^{a_i}}}{\partial a_j}  \\
	& = \frac{e^{a_k}e^{a_j}}{(\sum_i e^{a_i})^2} = \frac{e^{a_k}}{\sum_i e^{a_i}}\frac{e^{a_j}}{\sum_i e^{a_i}} = y_ky_j \text{ assumindo j } \neq k  \text{ ou,}\\
	& = \frac{e^{a_k}(\sum_i e^{a_i}) - (e^{a_k})^2}{(\sum_i e^{a_i})^2} \text{ assumindo j == k} \\
	& = \frac{e^{a_k}(\sum_{i \neq k} e^{a_i})}{(\sum_i e^{a_i})^2} = y_k(1 - y_k)
	\end{split}
	\end{equation}
\end{center}
...
as  classes serão $t_n \in \{-1, 1\}$ onde $t_n = 1$ se $x_n$ pertence à classe $C^1$ e
$t_n = -1$ se pertence à classe $C^2$.
...
\begin{center}
	\begin{equation}
		\begin{array}{ll@{}ll}
				\argmin_{w, b} & ||w||^2 &\\
				\textbf{sujeito a}& t_k(w^Tx_k + b) \ge 1, &&k = 1, \ldots ,n 
		\end{array}
	\end{equation}
\end{center}
...
Uma vez resolvido o problema dual e encontrado valor de b, pode0se classificar um novo
...
A abordagem usada neste trabalho para o caso multiclasses do SVM foi o método intuitivo chamado \textit{One-versus-all} (OVA).
 Nesse método são construídos K classificadores $y_k$ cada um definindo uma
...
cabeçalho da tabela representa o vocaulário.
