...
Ao longo deste capítulo será usada a letra n para se referir à quantidade de elementos
do conjunto de entrada, a i-ésima entrada é dada pelo vetor $x_i \in \mathbb{R}^m$.
O conjunto de entrada será referida como X
...
Na classificação temos K classes e cada elemento $x_i$ da entrada é associado a uma classe $t_i \in \{1..K\}$.
O objetivo do problema da classificação é dado entrada $X = (x_1, x_2, \ldots, x_n)$ 
e $t = (t_1, \ldots, t_n)$ treinar um modelo capaz de prever classes para uma entrada
qualquer.
...
a partir da classificação binária e, a partir dela, será descrito
como estender para o problema com mais de duas classes, que é o caso deste trabalho.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		$iteracao < maxIteracoes$ } \label{lst:line:condition}
			...
			- \sum_{i = 1}^{n} \{ t_nln(y_n) + (1 - t_n) ln(1 - y_n) \}$ \COMMENT{recalcula o erro}
...
Novamente pode-se encontrar o valor de W que minimize $E(W)$ usando os
...
\begin{enumerate}
	...
	Lagrangiano com as restrições
		\begin{dgroup}
			...
			\end{dmath}
		\end{dgroup}
	\item Deriva-se o lagrangiano com respeito a w, b e $\xi$ e igualamos a 0 para obter 
